...
BASE_PATH = os.path.abspath("C:\\Users\\marcu\\Desktop\\test")
CHECK_DELAY = 2 # Hur många sekunder mellan checks

def checkfiles():
    time.sleep(CHECK_DELAY)
    url = os.path.join(BASE_PATH,"*","*")
    files = glob.glob(os.path.join(BASE_PATH, "*", "*")) #hämtar alla paths på files
...
def send_contractShell(shellid, filepath, fileid):
    shell = Shell_recv.query.filter_by(id=shellid).first() #hämta skalkontrakt från databasen
    path = shell.path
    with open(path,'r') as readfile:
        contract = json.loads(readfile.read()) #läs in skalkontrakt
    contract["contractID"] = uuid.uuid1().int>>65
    ...
    try: #uppdatera databasen med nytt kontrakt
        ...
        send_contract(contract["contractID"], contract["clientID"]) #skicka kontraktet
    except:
        db.session.rollback()
        raise
    finally:
